...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM init_status WHERE script_name = 'story_data_v1.0') THEN
        ...
        -- 🔧 수정: STO와 STA 테이블 모두 존재 확인 (대문자)
        IF EXISTS (
            SELECT 1 FROM information_schema.tables
            WHERE table_schema = 'public' AND UPPER(table_name) = 'STO'
        ) AND EXISTS (
            SELECT 1 FROM information_schema.tables
            WHERE table_schema = 'public' AND UPPER(table_name) = 'STA'
        ) THEN
            ...
            VALUES ('story_data_v1.0', '스토리 데이터 초기화 완료');

            RAISE NOTICE '스토리 데이터 초기화 완료';
        ELSE
            RAISE NOTICE 'STO 또는 STA 테이블이 아직 생성되지 않았습니다';
        END IF;
    ELSE

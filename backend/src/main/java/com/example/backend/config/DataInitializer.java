...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initializeStations() {
        if (stationRepository.count() > 0) {
            log.info("역 데이터가 이미 존재합니다. 초기화를 건너뜁니다. ({}개)", stationRepository.count());
            return;
        }

        log.info("=== 역 데이터 초기화 시작 ===");
    ...
    private void initializeStoryData() {
        if (storyRepository.count() > 0) {
            log.info("스토리 데이터가 이미 존재합니다. 초기화를 건너뜁니다. ({}개)", storyRepository.count());
            return;
        }

        log.info("=== 스토리 데이터 초기화 시작 ===");
        ...
        try {
            ...
            log.info("=== 스토리 데이터 초기화 완료 ===");
            log.info("스토리: {} ({}페이지)", savedStory.getStoTitle(), pages.size());

...
export function middleware(request: NextRequest) {
  ...
  // refreshToken Cookie 확인 및 검증
  const refreshToken = request.cookies.get('refreshToken')?.value;

  // 토큰이 존재하고, JWT 형식인지 확인 (기본적인 형식 검증)
  const isValidFormat = refreshToken && /^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/.test(refreshToken);
  const isAuthenticated = Boolean(isValidFormat);
